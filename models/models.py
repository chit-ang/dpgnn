import sys
sys.path.append("D:\git\LDP-DG\models")
from layers import *
...
class DG(Model):
    ...
    def build_net(self, attn_head_config, attn_layer_config, temporal_head_config, temporal_layer_config,
        ...
        # 进入结构层
        input_list = self.placeholders['features']  #[143x 128]
        for layer in self.structural_attention_layers:
            attn_outputs = []
            for t in range(0, self.num_time_steps):
                out = layer([input_list[t], adjs[t]])
                attn_outputs.append(out)  #[1x 143 x 128]
        ...
        structural_outputs = tf.transpose(tf.concat(attn_outputs, axis=0), [1, 0, 2])  # [143, 16, 128]
        structural_outputs = tf.reshape(structural_outputs,
                                        [-1, self.num_time_steps, attn_layer_config[-1]])  # [143, 16, 128]

        # 进入时间层
        temporal_inputs = structural_outputs
        for temporal_layer in self.temporal_attention_layers:
            outputs = temporal_layer(temporal_inputs)  # [143, 16, 128]
    ...
    def _loss(self):
        ...
        for t in range(self.num_time_steps_train - num_time_steps_train, self.num_time_steps_train-1):
            output_embeds_t = tf.nn.embedding_lookup(tf.transpose(self.final_output_embeddings, [1, 0, 2]), t)
            inputs1 = tf.nn.embedding_lookup(output_embeds_t, self.placeholders['node_1'][t])#143,128
            inputs2 = tf.nn.embedding_lookup(output_embeds_t, self.placeholders['node_2'][t])#143,128
